...
public:
    ...
    DKtreeNeighbourIterator(const dktree *tree, Container_0 *C0, etype node) : tree(tree), node(node) {
        ...
        array<shared_ptr<ktree>, 8> k_collections = tree->k_collections();
        ...
        if (_c0_neigh_it != _c0_neigh_it_end) {
            ...
            for (_curr_ktree = 0; _curr_ktree <= tree->get_max_r(); _curr_ktree++) {
                if (!tree->is_invalid_tree(_curr_ktree)) {
                    _k_collection_it = tree->get_neighbour_begin(_curr_ktree, node);
...
private:
    ...
    etype node;
    value_type _ptr = -1;
    k2_tree_neighbour_iterator _k_collection_it;
