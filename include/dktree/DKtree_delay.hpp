...
#define R 8
    ...
    class DKtree_delay : public Graph<
            DKtreeEdgeIterator <
            DKtree_delay<k, t_bv, t_rank>, k2_tree<k, t_bv, t_rank>, edge_iterator<k2_tree<k, t_bv, t_rank>>>,
                         DKtreeNodeIterator<DKtree_delay<k, t_bv, t_rank>>,
                         DKtreeNeighbourIterator<DKtree_delay<k, t_bv, t_rank>, k2_tree<k, t_bv, t_rank>, neighbour_iterator<k2_tree<k, t_bv, t_rank>>, Container0,
    ...
    using dktree_edge_it = DKtreeEdgeIterator<DKtree_delay<k, t_bv, t_rank>, k_tree, k_tree_edge_it>;
    using dktree_node_it = DKtreeNodeIterator<DKtree_delay<k, t_bv, t_rank>>;
    using dktree_neighbour_it = DKtreeNeighbourIterator<DKtree_delay<k, t_bv, t_rank>, k_tree, k_tree_neighbour_it, Container0,
            Container0NeighIterator < Container0>>;
    private:
    uint max_r = 0;
        ...
        while (union_i <= union_final) {
            if (k_collection[union_i] != nullptr) {
                if (union_prev < R) {
                    ...
                    k_collection[union_i]->unionOp(tmp);
                    tmp.reset();
                }
                union_prev = union_i;
                ++union_i;
...
//        }
    ...
    bool operator==(const DKtree_delay<k, t_bv, t_rank> &rhs) const {
        bool eval = true;
