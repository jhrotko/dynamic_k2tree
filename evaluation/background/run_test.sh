...
eval_time() {
  ...
  for _ in $(seq 1 $RUNS); do
    ...
    ./add_delay_munro $DATASETDIR/$1/$1.tsv $1 >>"$1/$RUNS_FILE_DELAY_MUNRO"
  done
}
...
eval_memory() {
  ...
  /usr/bin/time -v --output="$1/mem_add_delay_munro_$1.txt" ./add_delay_munro "$DATASETDIR/$1/$1.tsv" $1
}
...
prepared_data() {
  for vertices in $(ls $DATASETDIR); do
    ...
    while read line_4; do
      sum_delay_munro=$(echo "$sum_delay_munro + $line_4" | bc)
    done < "$vertices/$RUNS_FILE_DELAY_MUNRO"
    ...
    TIME_DELAY_MUNRO+=("$(echo "scale=3; $sum_delay_munro / $RUNS" | bc)")
  ...
  for item in $(seq 0 $i); do
    echo "${X[${item}]} ${TIME_BACKGROUND[${item}]} ${MEMORY_BACKGROUND[${item}]} ${TIME[${item}]} ${MEMORY[${item}]} ${TIME_DELAY[${item}]} ${MEMORY_DELAY[${item}]} ${TIME_DELAY_MUNRO[${item}]} ${MEMORY_DELAY_MUNRO[${item}]}" >> $RUNS_DATA
  done
  i=$((i+1))
}
...
plot_data_time() {
  ...
  plot "$RUNS_DATA" using 1:2 with linespoints title "add edge parallel",\
       ...
       "$RUNS_DATA" using 1:8 with linespoints title "add edge munro"
EOF
}
...
plot_data_mem() {
  ...
  plot "$RUNS_DATA" using 1:3 with linespoints title "add edge parallel",\
       ...
       "$RUNS_DATA" using 1:9 with linespoints title "add edge munro"
EOF
}
