...
eval_time() {
  ...
  rm "$1/$RUNS_FILE"
  ./neighbor_iterator "$DATASETDIR/$1/$1-lists.tsv" $RUNS "../serialized/$1" >>"$1/$RUNS_FILE"
}

eval_memory() {
  echo "Evaluating memory for $1..."
  rm "$1/mem.txt"
  /usr/bin/time -v --output="$1/mem.txt" ./neighbor_iterator "$DATASETDIR/$1/$1-lists.tsv" 1 "../serialized/$1"
}
i=0
declare -a TIME=()
...
plot_data_time() {
  ...
  set xlabel '√m'
  set ylabel "t (µs)"
  plot "$RUNS_DATA-time-$TYPE" using 1:2 with linespoints pt 8 notitle
EOF
}
...
plot_data_mem() {
  ...
  plot "$RUNS_DATA-memory-$TYPE" using 1:2 with linespoints pt 8 notitle
EOF
}
